-- Apply UserFood feature to remote Supabase project (Postgres 15 compatible)
-- Safe/idempotent: uses conditional checks in pg_catalog and to_regclass
...
-- Grants (RLS will restrict data access)
...
-- 4) Ensure handle_new_user function exists (safe to replace)
...
-- 5) Ensure trigger exists to populate public.User on auth signups
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 6) Backfill public.User for any existing auth.users without a matching row
INSERT INTO public."User" (id, name)
